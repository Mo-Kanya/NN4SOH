...
class SingleRecord:
    ...
    def __repr__(self):
        return f"shape {self.data.shape} type {self}"
...
#                       for i in range(cycle_records[ref_idx].data.RW_cycles[idx].num_discharges)),axis=1
#             )
#         except Exception as e:
...
for ref_idx in range(len(cycle_records)):
    for idx in range(cycle_records[ref_idx].data.num_RW_cycles):
        ...
        charge_record.data = cycle_records[ref_idx].data.RW_cycles[idx].charge
        all_single_records.append(charge_record)
        # 分离放电记录
        try:
            num_single_records = cycle_records[ref_idx].data.RW_cycles[idx].num_discharges//squence_length
            for j in range(num_single_records):
                discharge_record = SingleRecord('D')
                discharge_record.data = np.concatenate(
                    tuple(cycle_records[ref_idx].data.RW_cycles[idx].discharge[i+j*squence_length] 
                        for i in range(squence_length)),axis=1
                        )
                all_single_records.append(discharge_record)
            # 如果剩下的够多，也重复一段放上
            if cycle_records[ref_idx].data.RW_cycles[idx].num_discharges % squence_length > squence_length*0.5:
                discharge_record = SingleRecord('D')
                discharge_record.data = np.concatenate(
                    tuple(cycle_records[ref_idx].data.RW_cycles[idx].discharge[-i] 
                        for i in range(squence_length)),axis=1
...
#         )
t = []
for i in range(len(all_single_records)):
        t.append(
            all_single_records[i].data[0,0]
        )
...
#         counter += 1
for i in range(len(all_single_records)):
    all_single_records[i].capacity = capacity[i]
...
temp_c = []
for i in range(len(all_single_records)):
    temp_c.append(all_single_records[i].capacity)
    temp_t.append(all_single_records[i].data[0,0])
plt.plot(t_p,capacity_p,'b')
plt.plot(t,capacity,'r')
