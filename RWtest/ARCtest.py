...
dataFile = '/home/mokanya/RWtest/B0005'   # Modify this path
...
for ktr in range(3):
    ...
    for epoch in range(num_epochs):
        for i in range(int(len(train_set) / batch_size[ktr])):
            img = []
            lb = []
            for j in range(batch_size[ktr]):
                img.append(train_set[i*batch_size[ktr] + j][0])
                lb.append(train_set[i * batch_size[ktr] + j][1])
            ...
            if ktr_in % 10 == 0:
                losses.append(loss)
            ktr_in += 10
    xs = np.array(data[400:460], dtype=np.float)
    xs = torch.FloatTensor(xs)
    xs = Variable(xs).cuda()
    ys = rnn(xs)
    yc = torch.FloatTensor(np.array(labels[400:460], dtype=np.float))
    yc = Variable(yc).cuda()
